...
## Buscar y analizar varios a la vez

Todo lo anterior es para un CVE, pero un analista casi nunca trabaja así. Por eso la herramienta tiene un par de formas más de usarla.

A veces no tienes el identificador del CVE, solo sabes que hay "algo de SQL injection en tal producto". Para eso está la búsqueda por descripción, que te deja llegar al CVE sin conocer su número de antemano.

Y la parte que más me importaba: poder meter varios CVEs y que los analice en lote. Lo analiza uno a uno, con pequeñas pausas entre llamadas para no chocar con los límites de las APIs, y al final saca una tabla comparativa con el score de cada uno. Eso es lo que de verdad se parece al día a día de un SOC, donde no llega un CVE sino diez a la vez y lo que necesitas es ver de un vistazo cuál atender primero. Aquí es donde el motor de scoring deja de ser un número suelto y se convierte en lo que buscaba desde el principio, una forma de ordenar un montón de vulnerabilidades por urgencia real.

Lo analizado se guarda en un historial dentro de la sesión, que además puedo exportar e importar como JSON para no perder el trabajo.
